...
describe("SemanticSBT contract", function () {
  ...
  */
  describe("mint and burn", function () {
    ...
    it("Mint more than one sbt with privacy ,should use diffrent object", async function () {
      const { semanticSBT, owner, addr1 } = await loadFixture(deployTokenFixture);
    ...
    it("Mint more than one sbt, both with privacy and withoud privacy", async function () {
      const { semanticSBT, owner, addr1 } = await loadFixture(deployTokenFixture);
