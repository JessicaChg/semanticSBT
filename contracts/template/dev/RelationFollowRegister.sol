...
contract RelationFollowRegister is IFollowRegister, SemanticSBTUpgradeable {
    ...
    function devFollow(address addr,address[] memory follower) external onlyMinter {
        IRelationFollow(_ownedFollowContract[addr]).devFollow(follower);
    }

    function devUnfollow(address addr,address[] memory follower) external onlyMinter {
        IRelationFollow(_ownedFollowContract[addr]).devUnfollow(follower);
    }
